...
public class HomeController {
	...
	public String home(MarketBoardVO vo, Model model) {
		//=========크롤링 날짜 지정=========
	            ...
	            for(int j=0;j<6;j++)
	            {
	               try{
	                  Document doc2=Jsoup.connect("https://www.daangn.com/"+link.get(j).attr("href")).get();
	                   ...
	                   vo.setBo_image(image);
	                   vo.setBo_content(content.text());

	                   fvo.setFile_original(image);
	                   fvo.setFile_category("0");
	                   //임의로 넣어주는 값들
	                   vo.setBo_category("");
	                   vo.setBo_regdate(d);
	                   vo.setBo_id(toString(j));
	                   vo.setBo_nickname("크롤링");
	                   vo.setBo_count(95L);
	      ...
	      model.addAttribute("he_list", service.boardListOrderByHeart());
	      model.addAttribute("rg_list", service.boardListOrderByRegdate());
