			...
			<div class="board_list_category">
				<div class="col-lg-12">
					<div class="inner-content">
						<span class="filter frm" data-oper="sell_btn">전체보기</span> <span>|</span>&nbsp;&nbsp;
				...
				<c:choose>
					...
					<tbody>
								...
								<c:forEach items="${sell}" var="sell" varStatus="status">
									...
									<tr>
										<%-- <input type="hidden" name="bo_num" value="${sell.bo_num}"> --%>
										<td scope="row" width="10%">
											<input type="hidden" name="bo_num" value="${sell.bo_num}">
											<c:out value="${sell.bo_num}" />
										</td>
										<td scope="row" width="20%">
											<div class=" frm sell_list_thumb" data-oper="thumb_btn">
												<img src="<spring:url value='/img/${sell.bo_image}'/>"
													width="40%" height="10%">
												<c:out value="${sell.bo_title}" />
											</div>
										</td>
										<td scope="row" width="20%"><c:out value="${sell.bo_price}" />원</td>
										<td scope="row" width="20%"><c:out value="<%=bo_ok%>" /></td>
										<td scope="row" width="20%"><fmt:formatDate
												pattern="yyyy/MM/dd" value="${sell.bo_regdate}" /></td>
									</tr>
								</c:forEach>
							</c:when>
							<c:otherwise>
	...
	$(document).ready(
		function() {
			...
			$('.frm').on("click",function(e) {
				...
				var bo_NumTag = $("input[name='bo_num']");
				var order = '<input type="hidden" name="align" value="regdate">';
					formObj.empty();
					formObj.append(bo_NumTag);
					formObj.append(order);
