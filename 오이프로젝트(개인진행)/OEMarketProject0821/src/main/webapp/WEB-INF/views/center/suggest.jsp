			...
			<c:when test="${fn:length(suggest) > 0}">
				<table class="table table-bordered table-secondary table-hover">
					...
					<c:forEach items="${suggest}" var="suggestList" varStatus="status">
						<%-- <input type="hidden" name="ce_num" id="ce_num" value="${suggest[status.index].ce_num}"> --%>
						<tr>
					        ...
					        <c:when test="${login.us_grade == 0 }">
	                              ...
	                              <td class="text-center"><button class="btn btn-danger center__tinybutton" id="deleteSuggestBtn" onclick="deleteSuggest()">삭제하기</button></td>
...
//폼 실행
	$(document).ready(
		...
		// 모달 & 글 삭제 ==============================================
			/*    $(".text-center button").on("click", function deleteSuggest(){
				   var formObj = $("#suggest_form");
			     	var modal = $("#myModal");
					//var ce_Num = ce_num.value;
					console.log("ce_Num"+ce_num);
		    	   modal.find("p").text("삭제 하시겠습니까?");
		           $("#myModal").modal("show");
		...
		//var ce_Num = $("input[name='ce_num']").clone();
	    	  ...
	    	  /*  $(".text-center button").on("click", function(e){
		    	   modal.find("p").text("삭제 하시겠습니까?");
		           $("#myModal").modal("show");
		           $("#modal-ok-btn").on('click',function(){
		           $("#myModal").modal("hide");
