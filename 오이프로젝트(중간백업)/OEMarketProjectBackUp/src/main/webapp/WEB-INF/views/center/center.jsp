...
<section>
	<div class="center__header__bottom">
		<div class="container">
			<div class="row">
				<div class="col-1">
					<p class="h6"><strong title="공지사항 목록으로" onclick="location.href='/center/notice'">공지사항</strong></p>
				</div>
				<div class="col-11">
					<p class="h6"><c:choose>
						<c:when test="${us_id != null }"><a href='/center/noticeDetails?us_id=<c:out value="${us_id }"/>&no_num=<c:out value="${noticeCenter.no_num }"/>'
						 style="text-decoration: none; color: black;"><c:out value="${noticeCenter.no_title }" /></a></c:when>
						<c:otherwise><a href='/center/noticeDetails?no_num=<c:out value="${noticeCenter.no_num }"/>'
						 style="text-decoration: none; color: black;"><c:out value="${noticeCenter.no_title }" /></a></c:otherwise>
					</c:choose></p>
				</div>
			</div>
		</div>
			...
			<tbody>
				<c:forEach items="${listCenter }" var="center">
					<tr>
						...
						<td style="padding-left: 15px;"><c:choose>
						<c:when test="${us_id != null }"><a href='/center/FAQDetails?us_id=<c:out value="${us_id }"/>&no_num=<c:out value="${noticeCenter.no_num }"/>'
						 style="text-decoration: none; color: black;"><c:out value="${center.no_title }" /></a></c:when>
						<c:otherwise><a href='/center/FAQDetails?no_num=<c:out value="${center.no_num }"/>'
						 style="text-decoration: none; color: black;"><c:out value="${center.no_title }" /></a></c:otherwise>
						</c:choose></td>
					</tr>
				</c:forEach>
			</tbody>
		...
		<c:choose>
			...
			<c:otherwise>
				...
				<div class="container" style="padding-top: 15px;">
					<div class="row">
						<div class="col-6">
							<div class="card mb-3 card__user" style="margin-left: -15px;">
						...
						<div class="col-6">
							<div class="card mb-3 card__user" style="margin-right: -15px;">
